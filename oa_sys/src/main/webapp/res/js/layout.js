layui.config({
	...
	//初始化顶级菜单
	$.getJSON(path + '/menu/getMenuTop?t=' + Math.random(), function(menuData) {
		...
		$menu.find('li.layui-nav-item').each(function() {
			...
			$this.on('click', function() {
				...
				//---------这是第一个栗子----------
				$.getJSON(path + '/menu/getMenuByPidRec',{id:id},function(data){
					navbar.set({
						elem: '#side',
						data: data
					});
		...
		//模拟点击顶级菜单的第一个菜单按钮
		$.getJSON(path + '/menu/getTopMenuFirst?t=' + Math.random(), function(id) {
